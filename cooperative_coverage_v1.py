"""
Cooperative Coverage Environment II (Two Directions, 24 robots, evaluation)
...
class CooperativeCoverage:
    def __init__(self, args):
        ...
        self.tar_row_num = 6
        self.tar_column_num = 4
        self.n_targets = self.tar_row_num*self.tar_column_num
    ...
    def reset(self):
        ...
        rot_mat1 = np.array([[np.cos((k+2)/2*np.pi), -np.sin((k+2)/2*np.pi)], [np.sin((k+2)/2*np.pi), np.cos((k+2)/2*np.pi)]])
        self.order = k
        ...
        self.tar_pos = copy.deepcopy(tar_pos)

        # reset positions of robots
        pos_1 = np.zeros((int(0.5*self.n_agents), self.dim))
        pos_1[0] = np.random.uniform([-3.0, 3.5], [3.0, 5.5], self.dim)

        for i in range(1, int(0.5*self.n_agents)):
            coll = 1  # collision: coll = 1; safe: coll = 0.
            while coll == 1:
                coll = 0
                pos_1[i] = np.random.uniform([-3.0, 3.5], [3.0, 5.5], self.dim)
        ...
        pos_2 = np.zeros((int(0.5*self.n_agents), self.dim))
        pos_2[0] = np.random.uniform([-3.0, 3.5], [3.0, 5.5], self.dim)

        for i in range(1, int(0.5*self.n_agents)):
            coll = 1  # collision: coll = 1; safe: coll = 0.
            while coll == 1:
                coll = 0
                pos_2[i] = np.random.uniform([-3.0, 3.5], [3.0, 5.5], self.dim)
                for j in range(self.n_agents):
                    if j < i:
        ...
        for i in range(int(0.5*self.n_agents)):
