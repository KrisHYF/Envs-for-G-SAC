"""
Cooperative Coverage Environment I (One direction, 12 (6) robots, training)
...
class CooperativeCoverage:
    def __init__(self, args):
        ...
        self.tar_row_num = 3  # 12:3, 6:2
        self.tar_column_num = 4  # 12:4, 6:3
    ...
    def reset(self):
        ...
        pos[0] = np.random.uniform([-3.0, 2.0], [3.0, 4.0], self.dim)  # default: [-3.0, 2.0], [3.0, 4.0]

        for i in range(1, self.n_agents):
            coll = 1  # collision: coll = 1; safe: coll = 0.
            while coll == 1:
                coll = 0
                pos[i] = np.random.uniform([-3.0, 2.0], [3.0, 4.0], self.dim)
                for j in range(self.n_agents):
                    if j < i:
                        if np.linalg.norm(pos[i] - pos[j]) < (self.safe_dis + 0.2):
                            coll = 1
                            break

        for i in range(self.n_agents):
            pos[i] = np.dot(rot_mat, pos[i])

        np.random.shuffle(pos)  # reorder
